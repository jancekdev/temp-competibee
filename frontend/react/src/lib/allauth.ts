// @ts-nocheck
// This code is taken from allauth.js library and is used to authenticate the user

import { getCSRFToken, initializeCSRFToken } from './cookies'
    ...
    if (path !== URLs.CONFIG) {
        if (settings.client === Client.BROWSER) {
            ...
            if (csrfToken) {
                ...
                console.debug('No CSRF token available for request to:', path)
            }
...
export async function requestEmailVerification(email) {
    try {
        ...
        const response = await request('POST', URLs.EMAIL, { email, action: 'resend_verification' });
        ...
        return {
            status: 500,
            errors: [{ param: 'email', message: 'Failed to request email verification' }]
        };
    }
}
